# Vorgegebene Concepte, Studierende sollen Verbindungen ausdenken
import importlib
import json
...
from cm_evaluator.models.metrics import Metric, MetricConfiguration, MislabeledConnectionsMetric, UnusedLabelsMetric
from cm_evaluator.utilities import words_are_similar
        ...
        for st_rel in student_relations:
            for m_rel in master_relations:
                if st_rel.conceptA == m_rel.conceptA and st_rel.conceptB == m_rel.conceptB and st_rel.relation_label != m_rel.relation_label:
                    # uses another template since its a label, not a word
                    if not words_are_similar(st_rel.relation_label, m_rel.relation_label, model, True):
...
def unused_edge_labels(
    ...
    if reference_config:
        ...
        score = 1 - (len(unused) / len(master_edge_labels))
